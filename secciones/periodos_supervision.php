...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['imagenes'])) {
    foreach ($_FILES['imagenes']['tmp_name'] as $index => $tmp_name) {
        ...
        $ruta_destino = "uploads/periodos_supervision/" . $nombre_archivo;

        if (!is_dir("uploads/periodos_supervision")) {
            mkdir("uploads/periodos_supervision", 0777, true);
        }

        if (move_uploaded_file($tmp_name, $ruta_destino)) {
            $stmt = $conn->prepare("INSERT INTO periodos_imagenes (periodo_id, ruta, descripcion) VALUES (?, ?, ?)");
            $stmt->execute([$periodo_id, $ruta_destino, $descripcion]);
        }
    }
}
